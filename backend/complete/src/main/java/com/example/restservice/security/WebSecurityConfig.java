    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable) // Disable CSRF as we are using Firebase tokens
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(HttpMethod.PUT, "/api/events/{id}").hasRole("ADMIN") // Allow admin to update events
                .requestMatchers(HttpMethod.DELETE, "/api/events/{id}").hasRole("ADMIN") // Allow admin to delete events

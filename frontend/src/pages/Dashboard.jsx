...
const Dashboard = () => {
  ...
  const isProfileIncomplete = () => {
    ...
    // Check if this is a regular registration user (has username set through registration)
    // Regular users should have all required fields from registration, so only show banner for OAuth users with missing info
    const hasUsername = userProfile.username && !userProfile.username.startsWith('parent_') && !userProfile.username.startsWith('volunteer_');
    ...
    // For users with auto-generated usernames (OAuth users), check for missing username
    const needsUsername = !hasUsername && (userProfile.username?.startsWith('parent_') || userProfile.username?.startsWith('volunteer_'));
    ...
    return hasCriticalMissing || needsUsername || (!hasContact && showProfileBanner);
  };
  ...
  const getMissingFields = () => {
    ...
    const hasAutoGeneratedUsername = userProfile.username?.startsWith('parent_') || userProfile.username?.startsWith('volunteer_');
  ...
  const handleBannerDismiss = () => {
    const criticalMissing = ['firstName', 'lastName'].some(field => !userProfile?.[field]);
    const hasAutoGeneratedUsername = userProfile.username?.startsWith('parent_') || userProfile.username?.startsWith('volunteer_');

    if (!criticalMissing && !hasAutoGeneratedUsername) {

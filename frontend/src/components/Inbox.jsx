...
const Inbox = ({ isOpen, onClose, isDropdown = false }) => {
  ...
  useEffect(() => {
    if (currentUser && currentUser.uid) {
      console.log('Setting up real-time message subscription for user:', currentUser.uid);

      const unsubscribe = firebaseRealtimeService.subscribeToUserMessages(
        currentUser.uid,
        (messages) => {
          console.log('Received real-time messages:', messages);
      ...
      return () => {
        if (unsubscribe) {
          ...
          firebaseRealtimeService.unsubscribe(`user_messages_${currentUser.uid}`);
        }
      };
    }
  }, [currentUser]);

  // Merge localStorage system messages with Firestore messages
  useEffect(() => {
    if (currentUser) {
  ...
  const markAsRead = async (messageId) => {
    ...
    if (message?.isSystem) {
      // Handle system messages (localStorage)
      const updatedMessages = messages.map(msg =>
        msg.id === messageId ? { ...msg, read: true } : msg
      );
      setMessages(updatedMessages);
      const systemMessages = updatedMessages.filter(msg => msg.isSystem);
  ...
  const deleteMessage = async (messageId) => {
    ...
    if (message?.isSystem) {
      ...
      try {
        ...
        console.log('Deleted Firestore message:', messageId);
      } catch (error) {
        console.error('Error deleting Firestore message:', error);
        // Fallback to local update if Firestore fails
        const updatedMessages = messages.filter(msg => msg.id !== messageId);
        setMessages(updatedMessages);
        const unread = updatedMessages.filter(msg => !msg.read).length;
